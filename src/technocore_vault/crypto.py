...
def base58btc_decode(text: str) -> bytes:
    ...
    for char in text:
        if char not in BASE58BTC_INDEX:
            raise VaultCryptoError(f"Invalid Base58 character: {char!r}")
...
def did_from_private_key(private_key: Ed25519PrivateKey) -> str:
    ...
    if len(multibase) != MULTIBASE_LENGTH or not multibase.startswith("z6Mk"):
        raise VaultCryptoError("Failed to derive valid Ed25519 did:key")
...
def public_key_from_did(did: str) -> Ed25519PublicKey:
    ...
    if not isinstance(did, str) or not did.startswith(prefix):
        raise VaultCryptoError("DID must start with 'did:key:'")
    multibase = did[len(prefix) :]
    if len(multibase) != MULTIBASE_LENGTH or not multibase.startswith("z6Mk"):
        raise VaultCryptoError("DID multibase must be 48 characters starting with z6Mk")
    raw = base58btc_decode(multibase[1:])
    if len(raw) != 34 or not raw.startswith(MULTICODEC_ED25519):
        raise VaultCryptoError("Decoded multicodec does not match Ed25519")
    try:
        return Ed25519PublicKey.from_public_bytes(raw[2:])
    except Exception as err:
        raise VaultCryptoError("Invalid public key material in DID") from err
...
def sign_bytes(private_key: Ed25519PrivateKey, payload: bytes) -> str:
    ...
    if not SIGNATURE_PATTERN.match(encoded):
        raise VaultCryptoError("Invalid base64url signature encoding produced")
    return encoded
...
def verify_bytes(did: str, signature: str, payload: bytes) -> bool:
    ...
    if not SIGNATURE_PATTERN.match(signature or ""):
        raise VaultCryptoError("Signature must be 86 base64url characters")
...
def create_identity(path: Path | str, passphrase: str) -> str:
    ...
    if target.exists():
        raise VaultCryptoError(f"Identity file already exists at: {target}")
    if not passphrase or len(passphrase) < 12:
        raise VaultCryptoError("Passphrase must contain at least 12 characters")
...
def load_identity(path: Path | str, passphrase: str | bytes) -> Ed25519PrivateKey:
    ...
    if not target.exists():
        raise VaultCryptoError(f"Identity key file not found: {target}")
    ...
    try:
        ...
        raise VaultCryptoError("Incorrect passphrase or corrupted identity key") from err

    if not isinstance(loaded, Ed25519PrivateKey):
        raise VaultCryptoError("Loaded key is not an Ed25519 private key")
    return loaded
...
def contribution_payload(artifact_url: str, commit: str) -> bytes:
    ...
    if not clean_url.startswith("https://"):
        raise VaultCryptoError("Artifact URL must be a valid HTTPS URL")
    if not COMMIT_PATTERN.match(clean_commit):
        raise VaultCryptoError("Commit must be a full 40- or 64-hexadecimal revision hash")
